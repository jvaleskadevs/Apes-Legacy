...
contract InheritApesBAKC {
	...
	// Hardcoded BAKC contract address. using Goerli GammaToken contract address for testing
	address internal _BAKC = 0xd60d682764Ee04e54707Bee7B564DC65b31884D0;
	//address internal _BAKC = 0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512; // hardhat
	...
	function approveApeTransferAfter(
		...
		require(_tokenId >= MIN_BAKC_ID && _tokenId <= MAX_BAKC_ID, "Invalid tokenId");
		// Get the BAKC interface
		IERC721 bakc = IERC721(_BAKC);
		// Get the owner of the tokenId
		address _apeHolder = bakc.ownerOf(_tokenId);
		// Check ownership of tokenId
		require(
			_apeHolder == msg.sender, "Caller is not token owner");
		// The caller is holding BAKC #tokenId, Great!
	...
	function claimApe(uint _tokenId) public {
		...
		// The caller is the beneficiary of BAKC #tokenId, Great!
		...
		bakc.safeTransferFrom(_apeHolder, _beneficiary, _tokenId);
		// Awesome, BAKC #tokenId has been succesfully transferred from the APE-Halla! PLW3!
	}
	...
	function approveTransferForAllApesAfter(
		...
		require(bakc.balanceOf(msg.sender) > 0, "Caller is not an ape holder");	
		// The caller is an ApeHolder, Great!
	...
	function claimAllApesFrom(address _apeHolder) public {
		...
		ERC721Enumerable bakc = ERC721Enumerable(_BAKC);
		uint totalApes = bakc.balanceOf(_apeHolder);
		for (uint i = 0; i < totalApes; i++) {
			uint tokenId = bakc.tokenOfOwnerByIndex(_apeHolder, 0);
			bakc.safeTransferFrom(_apeHolder, _beneficiary, tokenId);
		}
		// Awesome, BAKC #tokenIds has been succesfully transferred from the APE-Halla! PLW3!
	}
