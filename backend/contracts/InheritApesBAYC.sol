...
contract InheritApesBAYC {
	...
	// Hardcoded BAYC contract address. using Goerli AlphaToken contract address for testing
	address internal _BAYC = 0xF40299b626ef6E197F5d9DE9315076CAB788B6Ef;
	//address internal _BAYC = 0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512; // hardhat
	...
	function approveApeTransferAfter(
		...
		require(_tokenId >= MIN_BAYC_ID && _tokenId <= MAX_BAYC_ID, "Invalid tokenId");
		// Get the BAYC interface
		IERC721 bayc = IERC721(_BAYC);
		// Get the owner of the tokenId
		address _apeHolder = bayc.ownerOf(_tokenId);
		// Check ownership of tokenId
		require(
			_apeHolder == msg.sender, "Caller is not token owner");
		// The caller is holding BAYC #tokenId, Great!
	...
	function claimApe(uint _tokenId) public {
		...
		// The caller is the beneficiary of BAYC #tokenId, Great!
		...
		bayc.safeTransferFrom(_apeHolder, _beneficiary, _tokenId);
		// Awesome, BAYC #tokenId has been succesfully transferred from the APE-Halla! PLW3!
	}
	...
	function approveTransferForAllApesAfter(
		...
		require(bayc.balanceOf(msg.sender) > 0, "Caller is not an ape holder");	
	...
	function claimAllApesFrom(address _apeHolder) public {
		...
		ERC721Enumerable bayc = ERC721Enumerable(_BAYC);
		uint totalApes = bayc.balanceOf(_apeHolder);
		for (uint i = 0; i < totalApes; i++) {
			uint tokenId = bayc.tokenOfOwnerByIndex(_apeHolder, 0);
			bayc.safeTransferFrom(_apeHolder, _beneficiary, tokenId);
		}
		// Awesome, BAYC #tokenIds has been succesfully transferred from the APE-Halla! PLW3!
	}
